...
export default function SmoothScroll({children}: SmoothScrollProps) {
    ...
    useEffect(() => {
        ...
        const handleWheel = (e: WheelEvent) => {
            ...
            // Защита от конфликтов мыши после тачпада
            if (!isTrackpad) {
                const timeSinceTouchpadRelease = currentTime - touchpadLockReleaseTimeRef.current;
                if (timeSinceTouchpadRelease < settings.MOUSE_AFTER_TOUCHPAD_COOLDOWN) {
                    e.preventDefault();
                    return;
                }
            }
            ...
            const scrollStep = Math.sign(e.deltaY) * Math.max(Math.abs(e.deltaY), scrollSettings.minScrollStep);
            targetScroll += scrollStep;
