...
const BlogCard: React.FC<BlogCardProps> = ({ id, num, title, date, src }) => {
  return (
    <Link href={`/blogPage/${id}`}>
      ...
      >
        <div
          className={`!w-full h-[171px] rounded-tl-[8px] rounded-tr-[8px] relative`}
        >
          <Image
            src={src}
            alt={`Картинка ${num}`}
        ...
        <div className={`${styles.cardBody} py-[20px] px-[15px]`}>
          ...
          >
            <p
              className={`${styles.cardBodyDate}  flex items-center gap-[4px] text-[#737373] text-[12px] leading-[13px]`}
            >
              <Image
