...
//                     ref={(el) => {
//                         inputRef.current = el;
//                         register(inputName, { required }).ref(el); // Объединяем
//                     }}
//                     type={inputType}
...
//                     aria-label={title}
//                 />
//                 <span className={`${styles.titleTop} font-[Rubik] field__title ${errors[inputName] && '!text-[#FF3030]'} ${classNameTitle}`}>
//                   {title}
//                 </span>
...
const AppInput = forwardRef<HTMLInputElement, AppInputProps>(({
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (mask === 'date') {
            // Разрешаем только цифры, точки, backspace, delete, tab, стрелки
            const allowedKeys = [
    ...
    return (
        <div
            ...
            <label
                ...
                />
                <span
                    className={`${styles.titleTop} font-[Rubik] field__title ${errors[inputName] && '!text-[#FF3030]'} ${classNameTitle}`}>
                  {title}
                </span>
                <span className={`${styles.titleBottom} font-[Rubik]  field__title-top ${classNameTitle}`}>
                  {title}
