...
export default function ForgotPasswordPage() {
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            setCountdown((prev) => {
                if (prev === 1) {
                    clearInterval(timer);
                    setSubmitted(false); // Показываем форму снова
                    return 9; // Сброс на следующий раз
                }
                return prev - 1;
            });
        }, 1000);
    ...
    return (
        <div className={`relative flex gap-[30px] items-start justify-between  md:h-[467px]`}>
            <div>
                <FormProvider {...methods}>
                    {!submitted ? (
                        <form onSubmit={handleSubmit(onSubmit)} onClick={handleFormInteraction} className="space-y-4">
                            <AppInput
                        ...
                        <div className="md:w-[375px] text-[16px] text-[#adadad] text-center leading-[130%] space-y-4">
                            <p>Временный пароль отправлен.<br/>Рекомендуем после авторизации заменить пароль.</p>
                            <p className="text-[14px] text-[#878787]">Форма появится через {countdown} сек.</p>
                        </div>
                    )}
                </FormProvider>
            </div>
